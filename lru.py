...
MAX_CACHE_SIZE = 1000  # max number of cached entries
...
class LRUCacheManager:
    ...
    def add(self, key, query, response, embedding):
        # Add item to cache storage
        self.cache.data_manager.save(query, response, embedding)
        self.access_order[key] = query
        self.access_order.move_to_end(key)  # mark as most recently used

        # Evict least recently used if exceeding max_size
        if len(self.access_order) > self.max_size:
            lru_key, lru_query = self.access_order.popitem(last=False)
            print(f"Evicting LRU cache entry for query: {lru_query[:50]}...")
            self.cache.data_manager.delete(lru_query)  # delete from DB and vector index

    def get(self, key):
        # Move to most recently used if present
        if key in self.access_order:
            self.access_order.move_to_end(key)
            return True
        return False

def create_cached_llm_with_lru(cache: Cache, llm_func, max_cache_size=1000):
    SIMILARITY_THRESHOLD = 0.8
    lru_manager = LRUCacheManager(cache, max_cache_size)
    ...
    def cached_llm(query: str):
        ...
        if results:
            ...
            if best_score > SIMILARITY_THRESHOLD:
                # Use query as key for LRU
                key = query
                if lru_manager.get(key):
                    print("Cache hit!")
        ...
        key = query
        lru_manager.add(key, query, response, embedding)
        return response
...
if __name__ == "__main__":
    ...
    cached_llm = create_cached_llm_with_lru(gpt_cache_instance, get_llm_response, MAX_CACHE_SIZE)
